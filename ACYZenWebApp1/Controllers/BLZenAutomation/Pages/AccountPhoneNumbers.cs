...
public class AccountPhoneNumbers : Page
{
    ...
    public AccountPhoneNumbers(IWebDriver driver)
    {
        driver.Url = _url;
    }
    
    public AccountPhoneNumbers(IWebDriver driver, WebDriverWait wait)
    {
        _driver = driver;
        _wait = wait;
    }
    ...
    public async Task DeleteTelNomber(string Password, string idNum, string NewLogin)
    {
        ...
        await GetSmsCodee.TelNomberStatusSend(UiBase.Host, UiBase.ApiKey, idNum);
        //Получаем новый СМС
        Thread.Sleep(5000);
