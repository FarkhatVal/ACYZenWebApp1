...
using ACYZenWebApp1.Controllers.RefreshBlockChannel;
...
public class AuthPage : Page
{
    ...
    public AuthPage(IWebDriver driver, WebDriverWait wait)
    {
        _driver = driver;
        _wait = wait;
    }
        ...
        else
        {
                    ...
                    .Count > 0)
            {
                for (int m = 0; m < 3; m++)
                {
                    ...
                    telConfirm1.Click();
                    string apiTelStatusGetNewSms1 = $"/setStatus/?apiKey={UiBase.ApiKey}&status=send&idNum={idNum}";
                    var addressTelStatusGetNewSms1 = new Uri(UiBase.Host + apiTelStatusGetNewSms1);
                    ...
                    //Получаем новый СМС
                    Thread.Sleep(30000);
                    UiBase.ResponseGetPhoneNomber =
                        await GetSmsCodee.GetTelephoneNomber(UiBase.Host, UiBase.ApiGetPhoneNomber);
                    string codeNew1 = UiBase.ResponseGetSmsCode.SmsCode;
                    //ВВодим СМС код
            ...
            if (_driver.FindElements(By.XPath("//input[@data-t='field:input-question']")).Count > 0)
            {
                ...
                inputKv.SendKeys(UiBase.KvAnswer);
                    ...
                    .Count > 0)
            {
                ...
                GetSmsCodee.TelNomberStatusSend(UiBase.Host, UiBase.ApiKey, idNum);
                _driver.FindElement(By.XPath("//button[@data-t='button:action']")).Click();
                //Получаем новый СМС
                Thread.Sleep(30000);
                string codeNew = await GetSmsCodee.GetSmsCode(UiBase.Host, UiBase.ApiKey, idNum);
                if (codeNew == null)
                {
                    for (int n = 1; n <= 3; n++)
                    {
                        Thread.Sleep(5000);
                        codeNew = await GetSmsCodee.GetSmsCode(UiBase.Host, UiBase.ApiKey, idNum);
            ...
            {
                await BasicOperationForRefreshChannel.RefreshChannel(Login, UiBase.KvAnswer, null, null);
            }
        }
