...
public class BasicOperation : UiBase
{
    ...
    public static async Task InputAndConfirmTelNumber(IWebDriver Driver, WebDriverWait Wait, long? telNumber,
        string idNum)
    {
        ...
        if (Driver.FindElements(
                    By.XPath("//div[@class='error-message'][contains(text(), 'Недопустимый формат номера')]"))
                .Count > 0)
        {
                ...
                .Count > 0)
        {
            ...
            await GetSmsCodee.TelNomberStatusEnd(UiBase.Host, UiBase.ApiKey, idNum);
            var interTel2 = Driver.FindElement(By.XPath("//input[@data-t='field:input-phone']"));
            interTel2.SendKeys("\b\b\b\b\b\b\b\b\b\b");
            ResponseGetPhoneNomber = await GetSmsCodee.GetTelephoneNomber(UiBase.Host, UiBase.ApiGetPhoneNomber);
            ...
            {
                // Установка статутса номер уже использован, забанен на номер
                await GetSmsCodee.TelNomberStatusEnd(UiBase.Host, UiBase.ApiKey, idNum2);
                Driver.Close();
                Driver.Quit();
    ...
    public static async Task GetAndInputSmsCode(IWebDriver Driver, WebDriverWait Wait, string idNum, int? i)
    {
        Thread.Sleep(30000);
        for (int x = 0; x <= 2; x++)
        {
    ...
    public static async Task Auth(IWebDriver Driver, WebDriverWait Wait, int i, string Login, string Password,
